...
for(ii in 1:length(ll))
{
  ...
  for(jj in 1:length(mm))
  {
    ...
    ### predict
    mod <-MOD[[ll[ii]]][[mm[jj]]];
    bb <-aa[aa[,paste0(var,"_0")]==0,];
    cc <-aa[aa[,paste0(var,"_0")]==1 & aa$date>=min(bb$date),];
    kk <-1;
    for(kk in 1:nrow(cc))
    {
      ee <-cc[kk,];
      ff <-dd[dd$date%in%ee$date,c("date",qq)]; # regio days
      gg <-data.frame(var1=as.numeric(ff[,2]));
      pred <-predict(mod,newdata=gg,type = 'response',se = TRUE);
      Mm[[ll[ii]]][Mm[[ll[ii]]]$date%in%ee$date,var] <-pred$fit;
      Mm[[ll[ii]]][Mm[[ll[ii]]]$date%in%ee$date,paste0(var,"_0")] <-2;
      ### end kk  
    }
    ### end jj
  }
