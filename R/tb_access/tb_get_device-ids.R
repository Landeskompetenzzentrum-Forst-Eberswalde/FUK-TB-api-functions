...
G$d_in1 <-G$d_home;
G$d_out1 <-file.path(G$d_home,"output"); # file.path(aa[1],"output/level2",G$n_script); if(!dir.exists(G$d_out1)){dir.create(G$d_out1)};
### end
print(G)

# ENVIRONMENT -------------------------------------------------------
load_dot_env(file =file.path(G$d_out1,".env"))
E <-list();
